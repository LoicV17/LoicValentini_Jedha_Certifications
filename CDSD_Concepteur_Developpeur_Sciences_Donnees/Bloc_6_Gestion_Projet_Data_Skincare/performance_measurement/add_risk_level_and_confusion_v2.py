...
OUTPUT_CSV = os.path.join(CUR_DIR, "performance_summary_with_risk_v2.csv")
CONF_COUNTS_CSV  = os.path.join(CUR_DIR, "confusion_binary_risk_counts_v2.csv")
CONF_PERCENT_CSV = os.path.join(CUR_DIR, "confusion_binary_risk_percent_v2.csv")
CONF_FIG_PNG     = os.path.join(CUR_DIR, "confusion_binary_risk_v2.png")
...
def assign_risk(row):
    """Règles combinées :
       ...
       - FAIBLE si (p_bin < 0.20) ET (s_mal < 0.20) ET (classe ∈ {bkl, df, nv, vasc})
       - Sinon MODÉRÉ
    """
    ...
    if (p_bin_mal > 0.50) or (s_mal > 0.50) or (cls in MALIGNANT_CLASSES):
        return "ÉLEVÉ"
    if (p_bin_mal < 0.20) and (s_mal < 0.20) and (cls in BENIGN_CLASSES):
        return "FAIBLE"
    return "MODÉRÉ"
