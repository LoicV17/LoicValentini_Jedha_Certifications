...
MODEL_DIR = os.path.join(BASE_DIR, "models", "2_skindisease_mobilenetv2_finetuned")
...
# ⚙️ MobileNetV2 avec fine-tuning partiel
base_model = MobileNetV2(include_top=False, weights="imagenet", input_shape=(224, 224, 3))
base_model.trainable = True

# Dégeler uniquement les 20 dernières couches
...
# Callbacks
checkpoint_path = os.path.join(MODEL_DIR, "model2_best_model.h5")
callbacks = [
...
def plot_history(metric):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(MODEL_DIR, f"{metric}_curve_finetuned.png"))
    plt.close()
...
with open(os.path.join(MODEL_DIR, "classification_report_finetuned.txt"), "w") as f:
    f.write(report)
...
print("✅ Fine-tuning terminé. Résultats enregistrés dans :", MODEL_DIR)
