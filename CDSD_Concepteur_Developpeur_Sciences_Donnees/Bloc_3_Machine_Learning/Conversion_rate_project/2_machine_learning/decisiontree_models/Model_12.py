...
model12 = DecisionTreeClassifier(max_depth = 89, min_samples_split=20, min_samples_leaf=5, criterion='entropy')
...
# MLflow run
with mlflow.start_run(run_name="model12"):
    # Model training
    model12 = DecisionTreeClassifier(max_depth = 89, min_samples_split=20, min_samples_leaf=5, criterion='entropy')
    model12.fit(X_train, Y_train)
    ...
    mlflow.sklearn.log_model(model12, "model")
