...
y_proba = model19.predict_proba(X_test)[:, 1]
...
# MLflow run
with mlflow.start_run(run_name="model19"):
    # Model training

    model19.fit(X_train, Y_train)
    
    # Predictions on train set and test set and setting with new probability
    Y_train_pred = model19.predict(X_train)
    Y_train_pred_proba = model19.predict_proba(X_train)[:, 1]
    Y_train_pred = (Y_train_pred_proba >= best_threshold).astype(int)

    Y_test_pred = model19.predict(X_test)
    Y_test_pred_proba = model19.predict_proba(X_test)[:, 1]
    Y_test_pred = (Y_test_pred_proba >= best_threshold).astype(int)

    # Log the model and parameters
    mlflow.sklearn.log_model(model19, "model")
